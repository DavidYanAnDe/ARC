...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, attn_u, mlp_u, svd_dim = 50, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0., adp_drop = 0.5,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, tuning_mode='ARC'):
        super().__init__()
        self.dim = dim
        ...
        if('ARC' == tuning_mode):
            ...
            self.attn_lambda = nn.Parameter(torch.empty(1, svd_dim))
            self.attn_lambda_bias = nn.Parameter(torch.empty(self.dim))

            nn.init.zeros_(self.attn_lambda)
            nn.init.zeros_(self.attn_lambda_bias)

            self.mlp_lambda = nn.Parameter(torch.empty(1, svd_dim))
            self.mlp_lambda_bias = nn.Parameter(torch.empty(self.dim))

            nn.init.xavier_uniform_(self.mlp_lambda)
            nn.init.zeros_(self.mlp_lambda_bias)

            self.attn_drop = nn.Dropout(adp_drop)
            self.mlp_drop = nn.Dropout(adp_drop)
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        if('ARC' == self.tuning_mode):
            
            x = self.ARC_adapter(x, self.attn_u,  self.attn_drop, self.attn_lambda, self.attn_lambda_bias)
        ...
        if('ARC' == self.tuning_mode):
            ...
            x = self.ARC_adapter(x, self.mlp_u, self.mlp_drop, self.mlp_lambda, self.mlp_lambda_bias)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, svd_dim = 50,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, tuning_mode='ARC'):
        ...
        if('ARC' == tuning_mode):
            ...
            self.attn_u = nn.Parameter(torch.empty(dim, svd_dim))
            nn.init.xavier_uniform_(self.attn_u)
            
            self.mlp_u = nn.Parameter(torch.empty(dim, svd_dim))
            nn.init.xavier_uniform_(self.mlp_u)

        # build blocks
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim=dim, input_resolution=input_resolution, num_heads=num_heads, 
                attn_u = self.attn_u,mlp_u = self.mlp_u, svd_dim = svd_dim,
                window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop,
                drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer=norm_layer, tuning_mode=tuning_mode)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', tuning_mode='ARC', **kwargs):
        super().__init__()
